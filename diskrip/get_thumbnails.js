...
function getExistingThumbnails() {
    ...
    return fs.readdirSync(imgsDirectory)
 ...
 * Note: You can get a free API key from http://www.omdbapi.com/apikey.aspx
 */
async function searchMoviePoster(movieTitle) {
    // Try OMDB API (requires API key - returns empty if no key set)
    const omdbApiKey = process.env.OMDB_API_KEY || '';

    if (omdbApiKey) {
        try {
            const omdbUrl = `http://www.omdbapi.com/?apikey=${omdbApiKey}&t=${encodeURIComponent(movieTitle)}`;
            const data = await fetchJSON(omdbUrl);

            if (data && data.Poster && data.Poster !== 'N/A') {
                log(`Found poster for "${movieTitle}" via OMDB`);
                return data.Poster;
            }
        } catch (error) {
            log(`OMDB API error for "${movieTitle}": ${error.message}`);
        }
    }

    // Fallback: Try The Movie Database (TMDb) - no API key required for search
    // Note: For production use, you should get a free API key from https://www.themoviedb.org/settings/api
        ...
        try {
            ...
            if (searchData && searchData.results && searchData.results.length > 0) {
                ...
                if (posterPath) {
                    ...
                    log(`Found poster for "${movieTitle}" via TMDb`);
                    return posterUrl;
                }
            }
        } catch (error) {
            log(`TMDb API error for "${movieTitle}": ${error.message}`);
        }
    }

    log(`No poster found for "${movieTitle}" (tip: set OMDB_API_KEY or TMDB_API_KEY environment variable)`);
    return null;
}
...
async function convertAndCropImage(inputPath, outputPath) {
    return new Promise((resolve, reject) => {
        ...
        const convertArgs = [
            inputPath,
            '-resize', '900x600^',  // Resize to fit 3:2 ratio (900x600)
            '-gravity', 'center',    // Center the crop
            '-extent', '900x600',    // Crop to exact 3:2 ratio
            outputPath
        ];
        ...
        convert.on('error', (error) => {
            ...
            const ffmpegArgs = [
                '-i', inputPath,
                '-vf', 'scale=900:600:force_original_aspect_ratio=increase,crop=900:600',
                '-y',
                outputPath
            ];
            ...
            ffmpeg.on('error', (ffmpegError) => {
                reject(new Error('Neither ImageMagick nor ffmpeg available for image processing'));
            });
...
async function main() {
    ...
    if (!process.env.OMDB_API_KEY && !process.env.TMDB_API_KEY) {
        ...
        log('To fetch movie posters, set one of these environment variables:');
        log('  - OMDB_API_KEY (get free key at http://www.omdbapi.com/apikey.aspx)');
        log('  - TMDB_API_KEY (get free key at https://www.themoviedb.org/settings/api)');
        log('');
    }
